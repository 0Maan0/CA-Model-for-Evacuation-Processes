...
row = 100
corridor_lengths = [20, 40, 60, 80, 100]
phi_lists = []

# Choose a colormap (e.g., "plasma" or "viridis")
cmap = plt.colormaps.get_cmap("viridis")  
norm = mcolors.Normalize(vmin=min(corridor_lengths), vmax=max(corridor_lengths))

# Run simulations
for length in corridor_lengths:
    print(f"Corridor Length: {length}")
    number_agents = int(0.05 * row * length)
    print(f"Number of agents: {number_agents}")
    
    ffca = FFCA_wrap(row, length, number_agents, spawn_rate=0.025,
                     conflict_resolution_rate=0, alpha=0.3, delta=0.3,
                     static_field_strength=2.5, dynamic_field_strength=3,
                     horizontal_bias=5000)  
    phi_lists.append(lane_formation(row, length, number_agents, steps, ffca))

# Plot results
fig, ax = plt.subplots(figsize=(10, 6))

for i, phi_values in enumerate(phi_lists):
    color = cmap(norm(corridor_lengths[i]))  # Assign color based on corridor width
    ax.plot(range(steps), phi_values, linestyle='-', 
            color=color, linewidth=2, alpha=0.8, 
            label=f'Length: {corridor_lengths[i]}')
